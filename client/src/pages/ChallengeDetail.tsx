...
export const ChallengeDetail: React.FC<ChallengeDetailProps> = ({ 
  ...
  return (
    <div className="space-y-6 pb-16 animate-fadeIn">
      ...
      {/* Header section matching Mockup Screen 3 */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-[#18201E] tracking-tight">{challenge.title}</h1>
          <div className="flex items-center space-x-3 mt-1.5 text-xs text-[#7A8581]">
            ...
            <span className="px-2 py-0.5 rounded-full text-[10px] font-semibold bg-[#DDF2E7] text-[#16AF82]">
              {challenge.category}
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-3">
          <button
            ...
            className={`px-4 py-2 rounded-lg text-xs font-semibold flex items-center space-x-2 transition-all shadow-sm ${
              isSupported
                ...
                : 'bg-white text-[#16AF82] border border-[#16AF82] hover:bg-[#DDF2E7]'
            }`}
          >
            <ThumbsUp className="w-4 h-4" />
            <span>{isSupported ? 'Supported' : 'Support This Challenge'}</span>
          </button>
          ...
          <button
            ...
            className="px-3 py-2 bg-white text-[#7A8581] border border-[#E7EBE8] rounded-lg text-xs font-semibold hover:text-[#18201E] hover:bg-[#F5F6F4] transition-colors flex items-center space-x-1.5"
          >
      ...
      {/* Stats row */}
      <div className="grid grid-cols-3 gap-4 bg-white p-4 rounded-xl border border-[#E7EBE8] shadow-sm max-w-xl">
        <div className="flex items-center space-x-3">
          <div className="w-8 h-8 rounded-lg bg-[#DDF2E7] flex items-center justify-center text-[#16AF82]">
            <ThumbsUp className="w-4 h-4" />
          </div>
          <div>
            <p className="text-base font-bold text-[#18201E]">{supportersCount.toLocaleString()}</p>
            <p className="text-[11px] text-[#7A8581]">Supporters</p>
          </div>
        </div>

        <div className="flex items-center space-x-3 border-l border-[#E7EBE8] pl-4">
          <div className="w-8 h-8 rounded-lg bg-[#FEF3C7] flex items-center justify-center text-[#D97706]">
            <Clock className="w-4 h-4" />
          </div>
          <div>
            <p className="text-base font-bold text-[#18201E]">{challenge.daysLeft || 12}</p>
            <p className="text-[11px] text-[#7A8581]">Days Left</p>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-3 border-l border-[#E7EBE8] pl-4">
          ...
          <div>
            <p className="text-base font-bold text-[#18201E]">
              {challenge.affectedPeople ? `${(challenge.affectedPeople / 1000).toFixed(1)}k` : '2.5k'}
            </p>
            <p className="text-[11px] text-[#7A8581]">People affected</p>
          </div>
        </div>
      </div>
      ...
      {(() => {
        const activeSol = challenge.solutions && challenge.solutions.length > 0 ? challenge.solutions[0] : null;
        const displayStatus = activeSol?.status || (challenge.status === 'OPEN' ? 'PENDING_REVIEW' : challenge.status);
        const events = activeSol?.progressEvents && activeSol.progressEvents.length > 0 
          ? activeSol.progressEvents 
          : [
              {
                id: 'pe_sub',
                stage: 'SUBMITTED',
                title: 'Challenge Submitted to State Innovation Engine',
      ...
      {/* Tabs */}
      <div className="flex space-x-6 border-b border-[#E7EBE8] text-xs font-semibold">
          ...
          <button
            ...
            className={`pb-3 capitalize transition-colors border-b-2 ${
              activeTab === tab
                ...
                : 'border-transparent text-[#7A8581] hover:text-[#18201E]'
            }`}
          >
            {tab.replace('-', ' ')}
          </button>
        ...
        <div className="lg:col-span-7 space-y-6">
          {activeTab === 'overview' && (
            <div className="space-y-6">
              {/* Photo */}
              <div className="h-72 w-full rounded-2xl overflow-hidden border border-[#E7EBE8] shadow-sm">
                <img src={photoUrl} alt={challenge.title} className="w-full h-full object-cover" />
              </div>

              {/* Problem Description */}
              <div className="bg-white p-6 rounded-2xl border border-[#E7EBE8] shadow-sm space-y-3">
                <h3 className="font-bold text-base text-[#18201E]">Problem Description</h3>
                <p className="text-sm text-[#4A5552] leading-relaxed">
                  {challenge.description}
                </p>

                <div className="grid grid-cols-3 gap-4 pt-4 mt-4 border-t border-[#E7EBE8] text-xs">
                  <div>
                    <span className="text-[11px] text-[#7A8581] block">Location</span>
                    <span className="font-semibold text-[#18201E]">{challenge.location}</span>
                  </div>
                  <div>
                    <span className="text-[11px] text-[#7A8581] block">Category</span>
                    <span className="font-semibold text-[#18201E]">{challenge.category}</span>
                  </div>
                  <div>
                    <span className="text-[11px] text-[#7A8581] block">Submission Type</span>
                    <span className="font-semibold text-[#18201E]">Community Verified</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          {activeTab === 'ai-analysis' && (
            <div className="bg-white p-6 rounded-2xl border border-[#E7EBE8] shadow-sm space-y-4">
              <div className="flex items-center space-x-2 text-[#16AF82]">
                <Sparkles className="w-5 h-5" />
                <h3 className="font-bold text-base text-[#18201E]">AI Domain Assessment</h3>
              </div>
              <p className="text-xs text-[#7A8581]">
                Evaluated through deep semantic analysis across regional challenge taxonomies.
              </p>

              <div className="p-4 rounded-xl bg-[#F9FCFA] border border-[#E7EBE8] space-y-3 text-xs">
                <div className="flex justify-between items-center">
                  <span className="text-[#7A8581]">Categorization Confidence</span>
                ...
                <p className="text-[#4A5552] pt-2 leading-relaxed">
              ...
              {onNavigateToAIMatching && (
                <button
                  ...
                  className="w-full py-2.5 bg-[#16AF82] text-white rounded-lg text-xs font-semibold hover:bg-[#13976f] transition-colors flex items-center justify-center space-x-2"
                >
          ...
          {activeTab === 'collaborators' && (
            <div className="bg-white p-6 rounded-2xl border border-[#E7EBE8] shadow-sm space-y-4">
              <h3 className="font-bold text-base text-[#18201E]">Assigned Collaborators & Mentors</h3>
              <div className="space-y-3">
                <div className="p-4 rounded-xl border border-[#E7EBE8] flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    <div className="w-10 h-10 rounded-lg bg-[#DDF2E7] flex items-center justify-center text-[#16AF82]">
                      <GraduationCap className="w-5 h-5" />
                    </div>
                    <div>
                      <p className="text-xs font-bold text-[#18201E]">BIT Sindri (Environmental Lab)</p>
                      <p className="text-[11px] text-[#7A8581]">Student Team: Green Innovators (12 members)</p>
                    </div>
                  </div>
                  <span className="text-[11px] px-2 py-0.5 bg-[#DDF2E7] text-[#16AF82] font-semibold rounded-full">
                    Active Lead
                  </span>
                </div>
                ...
                <div className="p-4 rounded-xl border border-[#E7EBE8] flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <p className="text-xs font-bold text-[#18201E]">Tata Steel CSR & Foundation</p>
                      <p className="text-[11px] text-[#7A8581]">Engagement: Hardware + Solar Pumps Grant</p>
                    </div>
                  </div>
                  <span className="text-[11px] px-2 py-0.5 bg-[#E0F2FE] text-[#0284C7] font-semibold rounded-full">
                    Sponsor Partner
                  </span>
                </div>
              </div>
          ...
          {activeTab === 'comments' && (
            <div className="bg-white p-6 rounded-2xl border border-[#E7EBE8] shadow-sm space-y-4">
              <h3 className="font-bold text-base text-[#18201E]">Stakeholder Discussion</h3>
              ...
              <form onSubmit={handlePostComment} className="flex space-x-2">
                ...
                <button
                  type="submit"
                  className="px-4 py-2 bg-[#16AF82] text-white rounded-lg text-xs font-semibold hover:bg-[#13976f] flex items-center space-x-1"
                >
              ...
              {/* Comments Feed */}
              <div className="divide-y divide-[#E7EBE8] pt-2">
                {comments.map((c) => (
                  <div key={c.id} className="py-3.5 space-y-1">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-2">
                        ...
                        />
                        <span className="text-xs font-bold text-[#18201E]">{c.authorName}</span>
                        <span className="text-[10px] text-[#7A8581] bg-[#F5F6F4] px-2 py-0.5 rounded-full">
                          {c.authorRole}
                        </span>
                      </div>
                      <span className="text-[10px] text-[#7A8581]">{c.createdAt}</span>
                    </div>
                    <p className="text-xs text-[#4A5552] pl-8 leading-relaxed">{c.content}</p>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="lg:col-span-5 space-y-5">
          <div className="bg-white p-5 rounded-2xl border border-[#E7EBE8] shadow-sm space-y-4">
            <div className="flex items-center justify-between border-b border-[#E7EBE8] pb-3">
              <div className="flex items-center space-x-2 text-[#16AF82]">
                <Sparkles className="w-4 h-4" />
                <span className="text-sm font-bold text-[#18201E]">AI Analysis</span>
              </div>
              <span className="text-[11px] px-2 py-0.5 bg-[#DDF2E7] text-[#16AF82] font-bold rounded-full">
                Active
              </span>
            </div>

            <div className="space-y-2.5 text-xs text-[#7A8581]">
              <p className="font-semibold text-[#18201E]">AI pipeline processed:</p>
              <div className="space-y-1.5 pl-1">
                <div className="flex items-center space-x-2 text-[#18201E]">
            ...
            <div className="pt-3 border-t border-[#E7EBE8]">
              <span className="text-xs font-bold text-[#18201E] block mb-2">Required Skills</span>
              <div className="flex flex-wrap gap-1.5">
                {skillsList.map((skill, idx) => (
                  <span
                    key={idx}
                    className="px-2.5 py-1 rounded-md text-[11px] font-medium bg-[#F5F6F4] text-[#18201E] border border-[#E7EBE8]"
                  >
                    {skill}
                  </span>
                ))}
            ...
            <div className="p-3 bg-[#F9FCFA] rounded-xl border border-[#E7EBE8] text-[11px] text-[#4A5552] leading-snug">
              {challenge.aiClassification?.reasoning ||
