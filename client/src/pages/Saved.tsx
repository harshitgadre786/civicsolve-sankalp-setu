...
export const Saved: React.FC<SavedProps> = ({ onNavigate }) => {
  ...
  return (
    <div className="space-y-6 pb-16 animate-fadeIn">
      <div>
        <h1 className="text-2xl font-bold text-[#18201E] tracking-tight">Saved Items</h1>
          ...
          >
            <div>
              <div className="flex items-center justify-between">
                <span className="px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider bg-[#DDF2E7] text-[#16AF82]">
                  {item.itemType}
                </span>
                <button
                  onClick={() => handleRemove(item.id)}
                  className="text-[#7A8581] hover:text-red-500 p-1"
                >
                  <Trash2 className="w-3.5 h-3.5" />
                </button>
              </div>

              <h3 className="font-bold text-sm text-[#18201E] mt-2.5 line-clamp-1">{item.title}</h3>
              <p className="text-xs text-[#7A8581] mt-1 flex items-center space-x-1">
            ...
            <div className="pt-4 mt-4 border-t border-[#E7EBE8] flex items-center justify-between text-xs">
              <span className="text-[#7A8581] text-[11px]">{item.date}</span>
              <button
